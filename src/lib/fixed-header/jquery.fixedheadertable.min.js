  ...
  $.fn.extend({
    ...
    getRow: function(row) {
      ...
      $this.each(function(index2, item) {
        ...
        if (leftFixed.length > 0) {
          row.leftFixedRow = leftFixed
            .children(".fixed-table_body-wraper")
            .find("tr")
            .eq(index);
        }
        if (rightFixed.length > 0) {
          row.rightFixedRow = rightFixed
            .children(".fixed-table_body-wraper")
            .find("tr")
            .eq(index);
        }
    ...
    rowHover: function(cb) {
      ...
      return $this.each(function(index, item) {
        ...
        var $item = $(item),
          ...
          $fixedTableBodyTrs = $item
            .children(".fixed-table_body-wraper")
            .find("tr"),
          $leftFixed = $item.children(".fixed-table_fixed-left"),
          $rightFixed = $item.children(".fixed-table_fixed-right");
        $fixedTableBodyTrs
          .off("mouseenter.rowHover")
          .off("mouseleave.rowHover");
        $fixedTableBodyTrs
          .on("mouseenter.rowHover", _process)
          .on("mouseleave.rowHover", _process);
        if ($leftFixed.length > 0) {
          var $leftFixedTrs = $leftFixed
            .children(".fixed-table_body-wraper")
            .find("tr");
    ...
    _setFixedIndex: function() {
      return this.each(function() {
        var that = this,
          $this = $(this),
          $fixedTableHeaderTd = $this
            .children(".fixed-table_header-wraper")
            .find("th");
        if (this.fixedIndex) {
          return;
        }
        ...
        $fixedTableHeaderTd.each(function(index, th) {
          if (th.hasAttribute("data-fixed")) {
            var direction = ($(th).attr("data-direction") || "left")
              .toLowerCase();
  ...
  $.extend({
    calFixedColHeight: function(fixedTableBox) {
      ...
      var $fixedTableBox = $(fixedTableBox),
        $fixedTableHeader = $fixedTableBox.children(
          ".fixed-table_header-wraper"
        ),
      ...
      if (isIE) {
        maxHeight =
          ...
          $fixedTableBox[0].clientHeight ||
          $fixedTableBox[0].scrollHeight;
      } else {
        maxHeight = $fixedTableBox.height();
      }
      if (
        fixedTableBody.scrollWidth > fixedTableBody.clientWidth ||
        fixedTableBody.offsetWidth > fixedTableBody.clientWidth
      ) {
        hasCrosswiseScroll = true;
      } else {
      ...
      if (
        fixedTableBody.scrollHeight > fixedTableBody.clientHeight ||
        fixedTableBody.offsetHeight - $.getScrollWidth() >
          fixedTableBody.clientHeight
      ) {
      ...
      if (hasVerticalScroll) {
        ...
        if (
          $fixedTableBox.find(".fixed-table-box_fixed-right-patch").length == 0
        ) {
          var rightPatch = $(
              '<div class="fixed-table-box_fixed-right-patch"></div>'
            ),
            height = $fixedTableHeader.height();
          rightPatch.css({ width: scrollWidth2, height: height - 2 });
      ...
      $fixedTableBox
        .find(".fixed-table_fixed.fixed-table_fixed-right")
        .css("right", scrollWidth2 - 1 < 0 ? 1 : scrollWidth2 - 1);
    ...
    syncScroll: function(fixedTableBox) {
      ...
      var $fixedTableBox = $(fixedTableBox),
        ...
        $fixedTableBody = $fixedTableBox.children(".fixed-table_body-wraper"),
        $fixedCols = $fixedTableBox
          .children(".fixed-table_fixed")
          .children(".fixed-table_body-wraper");
    ...
    isIE: function() {
      ...
      if (
        /msie \d/g.test(ua) ||
        (/trident\/\d/g.test(ua) && /like gecko/g.test(ua))
      ) {
        return true;
      } else {
        return false;
